"""
Enterprise Discord Sender Service - FIXED VERSION
=================================================
Archivo: app/services/discord_sender.py

SOLUCIÓN COMPLETA para error 50109 - Invalid JSON
"""
...
class DiscordSenderEnhanced:
    """
    🚀 DISCORD SENDER ENTERPRISE - FIXED VERSION
    ===========================================
    
    ✅ SOLUCIÓN COMPLETA para error 50109
    ✅ Formato correcto para Discord webhooks
    ✅ Circuit Breaker + Rate limiting
    ✅ Fallback automático a enlaces
    """
    ...
    async def send_message_with_file(self, webhook_url: str, content: str, 
        ...
        """
        ✅ ENVIAR MENSAJE CON ARCHIVO - VERSIÓN CORREGIDA
        """
        if not webhook_url:
            return False
    ...
    async def _send_file_with_retry(self, webhook_url: str, content: str, 
                                  file_bytes: bytes, filename: str) -> bool:
        """
        ✅ ENVÍO DE ARCHIVO CON RETRY LOGIC - VERSIÓN CORREGIDA
        """
        
        for attempt in range(self.config['max_retries'] + 1):
            try:
                # ✅ SOLUCIÓN: Crear FormData correctamente sin payload_json
                ...
                # ✅ Agregar contenido como campo separado (NO como JSON)
                if content and content.strip():
                    data.add_field('content', content[:2000])
                
                # ✅ Agregar username como campo separado
                data.add_field('username', 'ReplicBot Enterprise')
                
                start_time = time.time()
                
                # ✅ SOLUCIÓN: Enviar sin headers de Content-Type (aiohttp los maneja automáticamente)
                async with self.session.post(webhook_url, data=data) as response:
                    response_time = time.time() - start_time
                    
                    # ✅ Discord responde con 200 para archivos (no 204)
        ...
        self.circuit_breaker.record_failure(webhook_url)
        logger.error(f"❌ Falló envío de archivo después de {self.config['max_retries']} intentos")
        return False
    
    def _get_content_type(self, filename: str) -> str:
        """Obtener content type basado en extensión de archivo"""
        # Usar mimetypes de Python
